...
function renderTaskOverview(task, id) {
  ...
  ).innerHTML = `<img src="/assets/img/Icon_delete.png" onclick="askBeforeDelete(${id})">`;
  document.getElementById(
    "editTaskContainerPrioPrio"
  ).innerHTML = `${task["prio"]} <img src="../assets/img/${task["prio"]}_white.png"/>`;
}
...
function checkEditedPrio() {
  if (
    typeof prioToEdit !== "undefined" &&
    prioToEdit !== null &&
    prioToEdit !== ""
  ) {
    return true;
  } else {
    document.getElementById(`prioAlertEdit`).innerHTML =
      "Please select a priority!";
  }
}

function preventBackgroundScroll() {
  document.getElementById("board").style = "max-height: 100vh; overflow:hidden";
}
