...
function checkIfProgressBarNeeded(subtaskCard, id) {
  ...
  if (subtaskCard.length > 0 && parentProgressbar != null) {
    ...
    parentProgressbar.style =
      "width: 120px; background-color: lightgray; border-radius: 8px;";
  }
  if (subtaskCard.length <= 0 && parentProgressbar != null) {
...
function renderBoardCard(
  ...
  if (cats != null) {
    ...
    board_todo.innerHTML += boardToDoTemplate(
      categoryCard,
      categoryColorCode,
      titleCard,
      descriptionCard,
...
function renderAssignmentIcons(assignedUser, contact, idContainer) {
  if (assignedUser === contact.user_name) {
    ...
    if (username != null) username.appendChild(newCircle);
  }
}
