...
function renderContacts(idContactContainer, mode) {
  document.getElementById(idContactContainer).innerHTML =
    templateContactSelection(mode);
  for (let i = 0; i < contacts.length; i++) {
    const contact = contacts[i];
    document.getElementById(`contactsOptions${mode}`).innerHTML +=
      templateContactsOptions(contact, i, mode);
  }
  document.getElementById(`contactsOptions${mode}`).innerHTML +=
    templateNewContact();
}
...
function templateContactsOptions(contact, i, mode) {
  ...
  <div id="listenContact" class="option contactList" onclick="handlecheckContactClick(event,${i},'${mode}')">
...
function updateAssignedContacts() {
  ...
  assignedContacts.forEach((contact) => {
    document.getElementById(
      "assignedContactIconsList"
    ).innerHTML += `<div class="board_Icons_Username" style="background-color: ${contact.color}">${contact.acronym}</div>
  </div>`;
  });
}
...
function checkAddSubTask(id, mode) {
  const subTaskName = document
    .getElementById(`inputSubtask${mode}`)
    .value.trim();

  if (subTaskName === "") {
    document.getElementById("subTaskAlertAdd").innerHTML =
      "Bitte einen Wert hinzufügen";
  } else {
    document.getElementById("subTaskAlertAdd").innerHTML = "";
    addSubTask(id, mode);
  }
...
function addCheck(index, id, mode) {
  ...
  if (existingImage) {
    ...
    document.getElementById(
      `checkBox${mode}${id}${index}`
    ).innerHTML = /*html*/ `<img src="assets/img/done-30.png">`;
